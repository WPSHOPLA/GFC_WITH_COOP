...
//get user locations
$userstates = $userClass->getSubscriptionLocations($uid);
$user_state_id_array = [];
foreach ($userstates as $userstate) {
    if ($userstate) {
        $user_state_id_array[] = $userstate->state_id;
    }
}

$extra_users = $userDetails->extra_users;
$package_id = $userDetails->sub_id;

//get bill address
$customer_id = $userDetails->co_customer_id;

$customer = $paymentClass->getCustomerById($customer_id)->customer;

$bill_addr = $bill_city = $bill_state = $bill_country = $bill_postcode = "";

if ($customer) {
    $bill_addr = $customer->bill_addr1;
    $bill_city = $customer->bill_city;
    $bill_state = $customer->bill_state;
    $bill_country = $customer->bill_country;
    $bill_postcode = $customer->bill_postcode;
}
...
?>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="sub_col">
                            ...
                            <form id="update_subscription_form" action="subscription_manage.php"
                                  method="post" class="sub_form">

                                <input type="hidden" name="total_price" id="total_price">
                                <input type="hidden" name="update_subscription" value="1">

                                <label class="text-info">You need to update your subscription info.</label>
                                <br>
                                <div class="control">
                                    ...
                                    <!--div class="col-xs-4 control">
                                        <div class="squaredOne">
                                            <input class="location_check location"
                                                <?php if (in_array($state['id'], $user_state_id_array)) { ?>
                                                    checked<?php } ?>
                                ...
                                <div class="form-group">
                                    <input type="text" name="cc_name" class="form-control" required
                                           placeholder="Name on card" id="cc_name">
                                </div>
                                ...
                                <div class="form-group">
                                    <select name="cc_country" class="form-control" id="cc_country"
                                            data-origin="<?php echo $bill_country; ?>"
                                ...
                                <div class="form-actions form-group ">
                                    <button type="button" id="update_subscription" class="pri_button full-width">
                                        UPDATE
                                    </button>
                                </div>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            //initialize cc_country
            var origin_country = $('#cc_country').attr('data-origin');
            if (origin_country) {
                $('#cc_country option').each(function () {
                    if ($(this).val() == origin_country) {
        ...
        function my_callback_function(code, message, response) {
            ...
            if (code == 200) {
                $('#update_subscription_form').submit();
            } else {
                alert(message);
            }
        }

        $('#update_subscription').click(function (e) {
            ...
            if (!$('#update_subscription_form').valid())
                return false;
