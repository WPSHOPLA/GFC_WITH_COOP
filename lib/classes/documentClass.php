...
class documentClass {
	...
	/* Add a PDF */
	public function addDocument($file_name_original, $file_name, $user_id, $state_id, $status, $sup_id = '') {
		try {
			...
			if ($count < 1) {
				...
				if($file_extension == 'pdf') {
					...
					$file_path = dirname((__DIR__)) . DIRECTORY_SEPARATOR  . 'documents' . DIRECTORY_SEPARATOR . basename($file_name, '.pdf');
				...
				$db = getDB();
				$stmt = $db->prepare("INSERT INTO documents (file_name_original, file_name, file_hash, user_id, date_creation, document_text, document_html, state_id, status) VALUES (:file_name_original, :file_name, :file_hash, :user_id, :date_creation, :document_text, :document_html, :state_id, :status)");
	...
	public function countDocumentsByState($conditions = []) {
		try {
			$db = getDB();
			...
			if(sizeof($conditions) == 0) {
				$stmt = $db->prepare("SELECT state_id, COUNT(*) FROM documents GROUP BY state_id");
			} else {
				$stmt = $db->prepare("SELECT state_id, COUNT(*) FROM documents WHERE " . $conditions_string . " GROUP BY state_id");
